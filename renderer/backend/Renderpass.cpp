...
bool Renderpass::init(VkRenderData* renderData) {
	...
	std::vector<VkAttachmentReference> attachmentRefs = {colorAttRef,
																											 selectionColorAttRef};
	...
	std::vector<VkAttachmentDescription> attachments = {colorAtt, selectionColorAtt, 
		depthAtt};
	...
	VkResult result =
			vkCreateRenderPass(renderData->rdVkbDevice.device, &renderPassInfo,
												 nullptr, &renderData->rdRenderpass);
...
void Renderpass::cleanup(VkRenderData* renderData) {
	...
	vkDestroyRenderPass(renderData->rdVkbDevice.device, renderData->rdRenderpass,
											nullptr);
}
