...
bool ComputePipeline::init(const VkRenderData& renderData,
                           VkPipelineLayout pipelineLayout,
                           VkPipeline* pipeline,
                           const std::string& computeShaderFilename) {
  /* shader */
  VkShaderModule computeModule =
      Shader::loadShader(renderData.rdVkbDevice.device, computeShaderFilename);
  ...
  VkResult result =
      vkCreateComputePipelines(renderData.rdVkbDevice.device, VK_NULL_HANDLE, 1,
...
void ComputePipeline::cleanup(const VkRenderData& renderData, VkPipeline pipeline) {
