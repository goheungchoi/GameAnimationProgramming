...
VkRenderer::VkRenderer(GLFWwindow* window) { mRenderData.rdWindow = window; }
...
void VkRenderer::hideMouse(bool bHide) { bHideMouse = bHide; }
...
bool VkRenderer::draw() {
	...
	if (mRenderData.rdHighlightSelectedInstance) {
		...
		mRenderData.rdSelectedInstanceHighlightValue = 0.5f;
	}
		...
		if (numInstances > 0) {
			...
			if (model->hasAnimations() && !model->getBoneList().empty()) {
				...
				for (unsigned int i = 0; i < numInstances; ++i) {
					...
					// selected instance highlight values
					if (currentSelectedInstance == instances.at(i)) {
						mSelectedInstance.at(instanceToStore + i).x =
					...
					InstanceSettings instSettings =
							instances.at(i)->getInstanceSettings();
					mSelectedInstance.at(instanceToStore + i).y =
							instSettings.instanceIndexPos;
				}
	...
	bufferResized |= ShaderStorageBuffer::uploadSSBOData(
			mRenderData, &mSelectedInstanceBuffer, (char*)mSelectedInstance.data(),
			mSelectedInstance.size() * sizeof(glm::vec2));
	...
	VkClearValue colorClearValue1;
	colorClearValue1.color = {{0.25f, 0.25f, 0.25f, 1.0f}};
	VkClearValue colorClearValue2;
	colorClearValue2.color = {0.f, 0.f, 0.f, 0.f};

	VkClearValue depthValue;
	depthValue.depthStencil.depth = 1.0f;

	std::vector<VkClearValue> clearValues = {colorClearValue1, colorClearValue2,
																					 depthValue};
...
bool VkRenderer::addModel(std::string modelFileName) {
	...
	if (mModelInstData.miAssimpInstances.size() == 2) {
		std::shared_ptr<AssimpInstance> firstInstance =
				mModelInstData.miAssimpInstances.at(1);
...
void VkRenderer::centerInstance(std::shared_ptr<AssimpInstance> instance) {
	glm::mat4 worldMat = instance->getWorldTransformMatrix();
	glm::vec3 modelPos(worldMat[3]);
	glm::vec3 modelFront(glm::normalize(worldMat[2]));

	mCamera->moveTo(modelPos + modelFront * 5.f + glm::vec3(0.f, 5.f, 0.f));
	mCamera->lookAt(modelPos);
}
...
void VkRenderer::cleanup() {
	...
	CommandBuffer::cleanup(mRenderData, mRenderData.rdCommandPool,
												 &mRenderData.rdCommandBuffer);
	CommandBuffer::cleanup(mRenderData, mRenderData.rdComputeCommandPool,
	...
	vkDestroyImageView(mRenderData.rdVkbDevice.device,
										 mRenderData.rdSelectionImageView, nullptr);
	vkDestroyImageView(mRenderData.rdVkbDevice.device,
										 mRenderData.rdLocalSelectionImageView, nullptr);

	vmaDestroyImage(mRenderData.rdAllocator, mRenderData.rdDepthImage,
									mRenderData.rdDepthImageAlloc);
	vmaDestroyImage(mRenderData.rdAllocator, mRenderData.rdSelectionImage,
									mRenderData.rdSelectionImageAlloc);
	vmaDestroyImage(mRenderData.rdAllocator, mRenderData.rdLocalSelectionImage,
									mRenderData.rdLocalSelectionImageAlloc);
...
bool VkRenderer::createDescriptorLayouts() {
...

	{
		...
		std::vector<VkDescriptorSetLayoutBinding> assimpBindings = {
				assimpUboBind, assimpSsboBind, assimpSsboBind2};
...
bool VkRenderer::updateDescriptorSets() {
	...
	/* we must update the descriptor sets whenever the buffer size has changed */
	{
		...
		selectionWriteDescriptorSet.descriptorType =
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
...

	{
		...
		selectionWriteDescriptorSet.descriptorType =
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
		selectionWriteDescriptorSet.dstSet =
				mRenderData.rdAssimpSkinningDescriptorSet;
...
bool VkRenderer::createSelectionImage() {
	VkExtent3D selectionImageExtent = {mRenderData.rdVkbSwapchain.extent.width,
																		 mRenderData.rdVkbSwapchain.extent.height,
																		 1};
	mRenderData.rdSelectionFormat = VK_FORMAT_R32_SINT;

	VkImageCreateInfo selectImageInfo{};
	selectImageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
	selectImageInfo.imageType = VK_IMAGE_TYPE_2D;
	selectImageInfo.format = mRenderData.rdSelectionFormat;
	selectImageInfo.extent = selectionImageExtent;
	selectImageInfo.mipLevels = 1;
	selectImageInfo.arrayLayers = 1;
	selectImageInfo.samples = VK_SAMPLE_COUNT_1_BIT;
	selectImageInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
	selectImageInfo.usage =
			VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;

	VmaAllocationCreateInfo selectionAllocInfo{};
	selectionAllocInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;
	selectionAllocInfo.requiredFlags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;

	VkResult result =
			vmaCreateImage(mRenderData.rdAllocator, &selectImageInfo,
										 &selectionAllocInfo, &mRenderData.rdSelectionImage,
										 &mRenderData.rdSelectionImageAlloc, nullptr);
	if (result != VK_SUCCESS) {
		Logger::log(
				1, "%s error: could not allocate selection buffer memory (error: %i)\n",
				__FUNCTION__, result);
		return false;
	}

	VkImageViewCreateInfo selectImageViewInfo{};
	selectImageViewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
	selectImageViewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
	selectImageViewInfo.image = mRenderData.rdSelectionImage;
	selectImageViewInfo.format = mRenderData.rdSelectionFormat;
	selectImageViewInfo.subresourceRange.baseMipLevel = 0;
	selectImageViewInfo.subresourceRange.levelCount = 1;
	selectImageViewInfo.subresourceRange.baseArrayLayer = 0;
	selectImageViewInfo.subresourceRange.layerCount = 1;
	selectImageViewInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;

	result =
			vkCreateImageView(mRenderData.rdVkbDevice.device, &selectImageViewInfo,
												nullptr, &mRenderData.rdSelectionImageView);
	if (result != VK_SUCCESS) {
		Logger::log(
				1,
				"%s error: could not create selection buffer image view (error: %i)\n",
				__FUNCTION__, result);
		return false;
	}

	/* local copy destination image */
	mRenderData.rdLocalSelectionFormat = mRenderData.rdSelectionFormat;

	selectImageInfo.tiling = VK_IMAGE_TILING_LINEAR;
	selectImageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
	selectImageInfo.usage = VK_IMAGE_USAGE_TRANSFER_DST_BIT;

	selectionAllocInfo.usage = VMA_MEMORY_USAGE_CPU_ONLY;
	selectionAllocInfo.requiredFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
																		 VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;

	result =
			vmaCreateImage(mRenderData.rdAllocator, &selectImageInfo,
										 &selectionAllocInfo, &mRenderData.rdLocalSelectionImage,
										 &mRenderData.rdLocalSelectionImageAlloc, nullptr);
	if (result != VK_SUCCESS) {
		Logger::log(1,
								"%s error: could not create local selection buffer image view "
								"(error: %i)\n",
								__FUNCTION__, result);
		return false;
	}

	selectImageViewInfo.image = mRenderData.rdLocalSelectionImage;
	selectImageViewInfo.format = mRenderData.rdLocalSelectionFormat;

	result =
			vkCreateImageView(mRenderData.rdVkbDevice.device, &selectImageViewInfo,
												nullptr, &mRenderData.rdLocalSelectionImageView);
	if (result != VK_SUCCESS) {
		Logger::log(
				1,
				"%s error: could not create local selection buffer image view (error: %i)\n",
				__FUNCTION__, result);
		return false;
	}
...
void VkRenderer::assignInstanceIndices() {
	for (size_t i = 0; i < mModelInstData.miAssimpInstances.size(); ++i) {
		InstanceSettings instSettings =
				mModelInstData.miAssimpInstances.at(i)->getInstanceSettings();
		instSettings.instanceIndexPos = i;
